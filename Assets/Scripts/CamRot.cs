...
public class CamRot : MonoBehaviour
{
    ...
    //Clamp Value
    private float minXRotAngle = -45; //min angle around x axis
    private float maxXRotAngle = 45; // max angle around x axis
 
    private float minCameraFieldOfView = 6;
    private float maxCameraFieldOfView = 15;
    ...
    void Start()
    {
        ...
        dir = new Vector3(0, sceneCamera.transform.position.y, distanceBetweenCameraAndTarget);//assign value to the distance between the maincamera and the target
    ...
    private void RotateCamera()
    {
        ...
        if (Application.isEditor || Application.platform == RuntimePlatform.WindowsPlayer)
        {
            Vector3 tempV = new Vector3(Mathf.Clamp(rotX,-45,5), rotY, 0);
